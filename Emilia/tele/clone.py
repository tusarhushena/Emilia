...
async def clone_bot(event):
    ...
    if not event.is_private:
        return await event.reply("Please clone **Harry** in your private chat.")
    user_id = event.sender_id
    check = await clone_db.find_one({"_id": user_id})
    if check:
        return await event.reply(
            "You have already cloned **Harry**. If you want to delete the clone, use `/deleteclone <bottoken>`"
        )
    bots = await clone_db.count_documents({})
    if bots > CLONE_LIMIT:
    ...
    if len(event.text.split()) == 1:
        return await event.reply(
            "Please provide the bot token from @BotFather in order to clone **Harry**.\n**Example**: `/clone 219218219:jksswq`"
        )
    token = event.text.split(None, 1)[1]
    check_token = await clone_db.find_one({"token": token})
    if check_token:
        return await event.reply("The same bot token has been used to clone **Harry**. Please use a different bot token.")
    time = await timer.find_one({"_id": user_id})
    if time:
        if (datetime.now() - time["time"]) < timedelta(hours=13):
            return await event.reply("You have recently deleted the cloned **Harry**. Please wait for 12 hours before cloning again.")
    ...
    try:
        try:
            ...
            LOGGER.error(f"An error occurred while testing the token: {e}")
            await wait.delete()
            await event.reply("The bot token you provided is incorrect. Please provide the correct bot token. If the token is correct, please wait a while and try again later (12 hours approximately). If the issue persists, contact @DeadlineTechSupport")
            return
        ...
        try:
            ...
            await event.reply(f"An error occurred while cloning **Harry**. Please try again or contact support @SpiralTechDivision.")
            await k.delete()
            await wait.delete()
            return
    except Exception as e:
...
async def delete_cloned(event):
    ...
    if not event.is_private:
        return await event.reply("Please delete Harry clone in your private chat.")
    user_id = event.sender_id
    check = await clone_db.find_one({"_id": user_id})
    if not check:
        return await event.reply(
...
async def set_startpic(event):
    ...
    if not get_info:
        return await event.reply("You have not cloned **Harry** yet. If you want to clone it, use `/clone <bottoken>` in @Elf_Robot private chat.")
    if not event.is_private:
...
async def broadcast(event):
    ...
    if not get_info:
        return await event.reply("You have not cloned **Emilia** yet. If you want to clone it, use `/clone <bottoken>` in @HarryClonerBot private chat.")
    if not event.is_private:
