...
example: /user ArshCypherZ"""
    ...
    try:
        ...
        if ufc is not None:
            ...
            auser = user
    if "user" not in query[0] and not (await AUTH_USERS.find_one({"id": auser})):
        return await message.reply_text(
            """Please connect your account first to use this cmd
Or connect your channel with /aniconnect cmd if you are anonymous""",
            reply_markup=InlineKeyboardMarkup(
                [
                    [
                        InlineKeyboardButton(
                            "Auth",
                            url=f"https://t.me/{BOT_USERNAME.replace('@', '')}/?start=auth",
                        )
                    ]
                ]
            ),
...
async def settings_cmd(client: Client, message: Message, mdata: dict):
    if message.chat.type == ChatType.PRIVATE:
        return await message.reply_text("Group and Channel Command Only.")
    cid = mdata["chat"]["id"]
    try:
        user = mdata["from_user"]["id"]
    except KeyError:
    ...
    if (
        user in DEV_USERS
        or status in [ADMINISTRATOR, CHAT_DEV_USERS]
            ...
            reply_markup=InlineKeyboardMarkup(
                [
                    [
                        InlineKeyboardButton(
                            "Logout",
                            url=f"https://t.me/{BOT_USERNAME.replace('@', '')}/?start=logout",
                        )
                    ]
                ]
            ),
...
async def nsfw_toggle_btn(client: Client, cq: CallbackQuery):
    ...
    try:
        ...
        await cq.answer()
        return
    if cus not in DEV_USERS and k == MEMBER:
        await cq.answer(
