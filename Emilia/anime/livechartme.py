    ...
    try:
        ...
        for i in db.findAll("item"):
            if (await B.find_one())["_id"] == str(i.find("title")):
                break
            if "Dub" not in str(i.find("title")):
                cr.append(
                    [
                        str(i.find("title")).split(" - "),
                        re.sub(r"<.*?>(.*)<.*?>", r"\1", str(i.find("guid"))),
...
{'**EP Title:** '+ii[1] if len(ii)==3 else ''}""",
            ...
            if len(ls[i]) == 3:
                ...
                msgssp.append(
                    [
                        ...
                        "https://nyaa.si/?q="
                        + re.sub(r" ", "%20", re.sub(r"(\().*?(\))", r"", i).strip()),
                    ]
                )
    except Exception:
        e = err()
    ...
    try:
        for i in dd.findAll("item"):
            ...
            if None not in [title, guid, url, enclosure]:
                hd.append(
                    [
    ...
    try:
        for i in de.findAll("item"):
            ...
            if None not in [title, guid, description, thumbnail]:
                mhd.append(
                    [
                        re.sub(r"<.*?>(.*)<.*?>", r"\1", title),
            ...
            async for id_ in HD_GRPS.find():
                ...
                try:
                    try:
                        ...
                        await clog("Emilia", i[0], "HEADLINES LINK")
    ...
    for i in lc_pin_data:
        if i["_id"] in lc_listed:
            ...
            lc_final_dict.append(
                {"_id": i["_id"], "pin": i["pin"], "current": [i["current"]]}
            )
    mal_final_dict = []
    mal_listed = []
    ...
    for i in lc_final_dict:
        ...
        if unpin_now:
            ...
            await HD_GRPS.find_one_and_update(
                tbud, {"$set": {"last": i["current"] + tbud["last"]}}
            )
